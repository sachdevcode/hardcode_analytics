...
router.get('/summary', async (_req: Request, res: Response): Promise<void> => {
  ...
  const { rows: totalRow } = await query<{ total: string | number }>(totalSql);
  ...
  `
    ...
    GROUP BY c.platform
  `;
  const { rows: platformRows } = await query<{ platform: string; count: string | number; avg_engagement: string | number }>(platformSql);
  ...
  `
    ...
    LIMIT 5
  `;
  const { rows: topRows } = await query<{ creator_id: string; handle: string; engagement_rate: string | number }>(topSql);
  ...
  const { rows: snapshotRows } = await query<{
    creator_id: string;
    handle: string;
    prev_engagement: string | number | null;
    curr_engagement: string | number | null;
  }>(alertsSql);
  const alerts = snapshotRows.map((r) => {
    ...
    return {
      ...
      alert_type: 'engagement_drop' as const,
    };
  });
...
router.get('/trend/:creatorId', async (req: Request, res: Response): Promise<void> => {
  ...
  const { rows } = await query<{
    ...
    views_total: string | number | null;
    video_count: string | number | null;
    avg_views_30d: string | number | null;
