...
const CREATOR_STUBS: { id: string; handle: string }[] = [
...
export async function seedCampaigns(): Promise<number> {
  await initDb();
  const isSqlite = useSqlite();
  const { rows } = await query<{ count: string | number }>(
    isSqlite ? 'SELECT COUNT(*) AS count FROM campaigns' : 'SELECT COUNT(*)::text AS count FROM campaigns'
  );
  if (parseInt(String(rows[0]?.count ?? '0'), 10) > 0) return 0;
  ...
  for (const c of CAMPAIGNS) {
    await query(
      isSqlite
        ...
        : `INSERT INTO campaigns (id, creator_id, brand, campaign_name, start_date, end_date, spend, conversions, status) VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9) ON CONFLICT (id) DO NOTHING`,
      [c.id, c.creator_id, c.brand, c.campaign_name, c.start_date, c.end_date, c.spend, c.conversions, c.status]
    );
  }
