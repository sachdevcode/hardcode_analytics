const YOUTUBE_CHANNEL_SEED: { channelId: string; handle: string; category: string }[] = [
...
async function fetchWithBackoff(
  url: string,
  retries = 3
): Promise<{ ok: boolean; status: number; body: string; data?: { items?: YouTubeChannelStats[] } }> {
  ...
  for (let attempt = 0; attempt <= retries; attempt++) {
    try {
      ...
      let data: { items?: YouTubeChannelStats[] } | undefined;
      try {
        data = JSON.parse(body) as { items?: YouTubeChannelStats[] };
      } catch {
...
export async function fetchChannelStats(
  ...
  let avgViews30d: number | null = null;
...
export function getSeedChannels(): { channelId: string; handle: string; category: string }[] {
  return [...YOUTUBE_CHANNEL_SEED];
}

export async function runIngestion(apiKey: string): Promise<number> {
  if (!apiKey?.trim()) {
  ...
  for (const { channelId, handle, category } of seed) {
    ...
    if (!fetched) continue;
    await query(
      `INSERT INTO creators (id, platform, handle, display_name, channel_id, category, created_at)
      ...
      [fetched.creatorId, fetched.handle, fetched.displayName, fetched.channelId, category, now]
    );
    await query(
      `INSERT INTO snapshots (creator_id, fetched_at, subscribers, views_total, video_count, avg_views_30d, engagement_rate, raw_json)
